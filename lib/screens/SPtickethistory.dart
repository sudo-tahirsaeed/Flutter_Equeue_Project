...
class Ticket {
  ...
  factory Ticket.fromJson(Map<String, dynamic> json) {
    ...
    int ticketId =
        0; // Default value, you can change it to any suitable default value
    ...
    switch (statusValue) {
      ...
      default:
        status = TicketStatus
            .waiting; // Set a default status in case of unexpected value
        break;
    }
    // int id=12;
    DateTime dateTime =
...
class SPTicketHistory extends StatefulWidget {
  final String serviceid;

  SPTicketHistory({required this.serviceid});

  @override
  _TicketHistoryState createState() => _TicketHistoryState();
}

class _TicketHistoryState extends State<SPTicketHistory>
  ...
  Future<void> logout() async {
    ...
    final body1 =
        json.encode({'serviceid': widget.serviceid, 'type': 'company'});
  ...
  void initState() {
    ...
    fetchUserAllTickets(widget.serviceid);
  }
  ...
  Future<void> fetchUserAllTickets(String serviceid) async {
    setState(() {
      _isLoading = true; // Set isLoading to true before making the request
    });

    final url =
        Uri.parse('http://192.168.18.154:3000/Companyhistoryalltickets');
    final response = await http.post(
      url,
      headers: {'Content-Type': 'application/json'},
      body: json.encode({'userid': serviceid}),
    );
    print('id: ' + serviceid);
    ...
    setState(() {
      _isLoading = false; // Set isLoading to true before making the request
    });
  }
...
class TicketCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    bool showCancelButton =
        false; // Flag to determine if Cancel button should be shown

    switch (ticket.status) {
      case TicketStatus.waiting:
        statusColor = const Color.fromARGB(255, 249, 227, 27);
        statusText = 'Waiting';
        showCancelButton = false; // Show Cancel button for Waiting status
        break;
